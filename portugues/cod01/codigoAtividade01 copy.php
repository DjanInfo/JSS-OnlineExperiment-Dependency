...
#ifdef CONTROLE_IDADE
const int limite_idade_para_vacinar =  40;
#endif
...
void vacinar() {
	...
	int i, n=4;
    for ( i = 0; i < n; i++ ){
		if ( p[i].vacinado == false ){
			#ifdef CONTROLE_IDADE
			if ( p[i].idade > limite_idade_para_vacinar ){
			#endif
				#ifdef CONTROLE_GESTANTE
				if ( !p[i].gestante ){
				#endif
					...
					break;
						...
						printf("Nao pode vacinar gestantes!!! \nChamando Proximo da fila\n\n");
					#endif	
					#ifdef CONTROLE_IDADE
					} else
